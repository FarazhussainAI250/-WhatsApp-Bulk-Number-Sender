...
if st.button("🔗 Generate WhatsApp Links"):
    if not numbers:
        ...
        for i, num in enumerate(numbers, start=1):
            ...
            if len(clean_num) < 10:
                st.warning(f"Invalid number format: {num}")
                continue
            encoded_message = urllib.parse.quote(message)
            whatsapp_url = f"https://wa.me/{clean_num}?text={encoded_message}"
            st.markdown(f"**{i}.** [{clean_num}]({whatsapp_url}) - [Open WhatsApp]({whatsapp_url})")
        
        # Download links as HTML with proper escaping
        import html
        ...
        <body>
            ...
            <p><strong>Message:</strong> {escaped_message}</p>
            <hr>
        """
        ...
        for i, num in enumerate(numbers, start=1):
            ...
            encoded_message = urllib.parse.quote(message)
            whatsapp_url = f"https://wa.me/{clean_num}?text={encoded_message}"
            html_content += f'<div class="link">{i}. <a href="{html.escape(whatsapp_url)}" target="_blank">{html.escape(clean_num)} - Open WhatsApp</a></div>\n'
